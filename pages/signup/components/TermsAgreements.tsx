...
function TermsAgreements({ progressStatus }: SignupContentProps) {
  const { checkedTerms, handleSingleCheck, handleAllCheck, validateTermCheck } =
    useHandleTermsCheck({
      progressStatus,
    });
      ...
      {TERMS.map((termContent, key) => {
        return (
          <div key={key}>
            ...
            <p
              className="border-zinc-400 h-200 w-675 overflow-y-auto rounded-[10px] border p-20"
              key={key}
            >
              {termContent.content}
            </p>
            <label className="m-10 mb-50 flex items-center gap-12">
              <input
                ...
                className="size-18 appearance-none"
                onChange={(e) =>
                  handleSingleCheck(e.target.checked, termContent.termNo)
                }
